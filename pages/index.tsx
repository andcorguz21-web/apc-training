    ...
    <>
      ...
      <main className="lp" style={{ ["--ink" as any]: brand.brand_ink, ["--primary" as any]: brand.brand_primary }}>
        ...
        <section className="lp-hero">
          <span className="mono eyebrow"><i className="pulse" /> Hybrid Training</span>
        ...
        <div className="lp-marquee" aria-hidden>
          <div className="lp-track">
            {Array.from({ length: 2 }).map((_, i) => (
              <span key={i}>Entrenamiento funcional <b>·</b> Grupos pequeños <b>·</b> Rutinas 1:1 <b>·</b> Mensualidad o sesión suelta <b>·</b> Seguimiento personalizado <b>·</b>&nbsp;</span>
            ))}
          </div>
        </div>
        ...
        <footer className="lp-foot">
          ...
          <span className="mono dim">© {new Date().getFullYear()} Golplay.app</span>
        </footer>
...
.lp-aurora{position:fixed;top:-30%;right:-15%;width:75vw;height:75vw;z-index:0;pointer-events:none;
  background:radial-gradient(circle,var(--primary,#16a34a) 0%,transparent 60%);filter:blur(60px);opacity:.20;animation:auroraA 16s ease-in-out infinite alternate}
...
.lp-ghost{position:fixed;right:-3vw;bottom:-9vh;z-index:0;pointer-events:none;user-select:none;
...
.lp-hero{flex:1;display:flex;flex-direction:column;justify-content:center;max-width:1000px;padding:7vh 0 4vh}
.eyebrow{display:inline-flex;align-items:center;gap:9px;color:var(--primary,#16a34a);margin-bottom:26px;opacity:0;animation:rise2 .7s .05s forwards}
.pulse{width:7px;height:7px;border-radius:50%;background:var(--primary,#16a34a);animation:pulse 2.2s infinite}
.lp-title{font-family:'Archivo',sans-serif;font-weight:900;font-size:clamp(52px,12.5vw,168px);line-height:.84;letter-spacing:-.04em;opacity:0;animation:rise2 .85s .12s forwards}
.lp-title em{font-style:normal;color:var(--primary,#16a34a)}.lp-title .per{color:var(--primary,#16a34a)}
.lp-sub{font-size:clamp(16px,2.1vw,21px);line-height:1.5;color:#B9B7AF;max-width:480px;margin-top:30px;opacity:0;animation:rise2 .85s .22s forwards}
.lp-cta{display:flex;flex-wrap:wrap;gap:13px;margin-top:38px;opacity:0;animation:rise2 .85s .32s forwards}
...
.lp-marquee{margin:2vh calc(clamp(18px,3.4vw,40px) * -1) 0;border-top:1px solid rgba(236,234,227,.1);border-bottom:1px solid rgba(236,234,227,.1);overflow:hidden;padding:13px 0}
